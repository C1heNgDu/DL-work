...
class MASK_RCNN(object):
    _defaults = {
        # "model_path": 'model_data/hh/best_epoch_weights.h5',
        # 1arborizing vessels 2blue-grey globules 3blue-grey ovoid nests 4shiny white structures 5ulceration
        # 1/best_epoch_weights-0216 2/best_epoch_weights-0203 3/best_epoch_weights-0203 4/best_epoch_weights 5/best_epoch_weights-0227
        "model_path": 'logs/1/best_epoch_weights-0216.h5',
    ...
    def __init__(self, **kwargs):
        ...
        if self.num_classes <= 81:
            ...
            self.colors = np.array([[0, 0, 0], [0,171,216], [233, 30, 99], [156, 39, 176], [103, 58, 183],
    ...
    def detect_image(self, image):
        ...
        for i in range(np.shape(class_ids)[0]):
            ...
            cv2.rectangle(image_origin, (left, top), (right, bottom), color, thickness)

            # 获得这个框的种类并写在图片上
            class_name = self.class_names[class_ids[i]]
            print(class_name, top, left, bottom, right)
            # text_str = f'{class_name}: {class_thre[i]:.2f}'
            text_str = f'{class_thre[i]:.2f}'
            text_w, text_h = cv2.getTextSize(text_str, font, scale, 1)[0]
            cv2.rectangle(image_origin, (left, top), (left + text_w, top + text_h + 5), color, -1)
            cv2.putText(image_origin, text_str, (left, top + 15), font, scale, (255, 255, 255), 1, cv2.LINE_AA)

        image = Image.fromarray(np.uint8(image_origin))
        return image
